...
resource "aws_route_table" "tfPrivateRT2" {
  vpc_id = aws_vpc.tfVPC.id
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.tfNatGateway2.id
  }
  tags = {
    Name    = "tfPrivateRT2"
    Project = "TF Project"
...
resource "aws_route_table_association" "tfPrivateRTassociation1" {
  subnet_id      = aws_subnet.tfPrivateSubnet1.id
  route_table_id = aws_route_table.tfPrivateRT1.id
}
resource "aws_route_table_association" "tfPrivateRTassociation2" {
  subnet_id      = aws_subnet.tfPrivateSubnet2.id
  route_table_id = aws_route_table.tfPrivateRT2.id
}
